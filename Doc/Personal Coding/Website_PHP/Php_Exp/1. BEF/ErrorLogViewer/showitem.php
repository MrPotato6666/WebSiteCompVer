...
$fileopened=false;
if (($handle = fopen("d:\\WebRoot\\ErrorLog\\$filename", "r"))&&(($data = fgetcsv($handle, 10000, ",")) !== FALSE))
	$fileopened=true;
else
	$fileopened=false;
...
if (count($data)>=6)
{
	...
	if (($mode==0)&&($bFound))
	{
		if (($data[5]!="Normal")&&($data[5]!="Normal(BypassChecking)"))
			$bFound=false;
	}
	else if (($mode==1)&&($bFound))
	{
		if (($data[5]!="Motion Only")&&($data[5]!="Motion Only(BypassChecking)"))
			$bFound=false;
	}
	if ($bFound)
	{
